...
bool Texture::LoadTextureFromFile(ID3D11Device * device)
{
	...
	// 셰이더 리소스 뷰 만들기
	device->CreateShaderResourceView(texture, &viewDesc, &textureResourceView);
...
HRESULT Texture::GetScratchImage(LPCTSTR fileName, ScratchImage * image)
{
	...
	// 이미지 파일 확장자 확인
	if (wcscmp(fileExtension, TEXT(".tga")) == 0 || (fileExtension, TEXT(".TGA")) == 0)
		return LoadFromTGAFile(fileName, NULL, *image);
	else if (wcscmp(fileExtension, TEXT(".dds")) == 0 || (fileExtension, TEXT(".dds")) == 0)
