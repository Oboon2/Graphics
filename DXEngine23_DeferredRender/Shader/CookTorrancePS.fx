...
float4 main(ps_input input) : SV_TARGET
{
    ...
    float3 diffuse = OrenNayer(input.position, input.lightPosition, input.viewPosition, roughness, input.normal);
    float3 specular = CookTorrance(input.position, input.lightPosition, input.viewPosition, input.normal, roughness);
    float3 texColor = diffuseMap.Sample(diffuseSampler, input.texCoord).rgb;
    // Diffuse + Specular + Ambient <= 1
    float3 ambient = float3(0.1f, 0.1f, 0.1f);
    //float3 final = diffuse * texColor + ambient * texColor + specular * texColor;
