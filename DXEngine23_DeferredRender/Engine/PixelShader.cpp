...
bool PixelShader::CompileShader(ID3D11Device * device)
{
	...
	// 픽셀 셰이더 컴파일 -> 바이트 코드
	HRESULT result = D3DCompileFromFile(fileName, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, entryPoint, profile, NULL, NULL, &shaderBuffer, NULL);
...
void PixelShader::Release()
{
	...
	for (unsigned int ix = 0; ix < textures.size(); ++ix)
	{
		textures[ix].Release();
	}
